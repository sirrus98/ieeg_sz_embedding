...
class LightweightSeizureDetector(nn.Module):
    ...
    def get_features(self, x):
        ...
        Args:
            x: [batch, 1, 256 or 2560] - single electrode signal
            
        Returns:
            features: [batch, base_filters] - pooled features
        """
        # Stem
        ...
        # Compress
        x = self.compress(x)  # [batch, base_filters//2, seq_len//2]
        
        # Global pooling
        gap = self.gap(x).squeeze(-1)  # [batch, base_filters//2]
        gmp = self.gmp(x).squeeze(-1)  # [batch, base_filters//2]
        features = torch.cat([gap, gmp], dim=1)  # [batch, base_filters]
    ...
    def get_seizure_probability_temp(self, x):
        """
        Convenience method for inference.
